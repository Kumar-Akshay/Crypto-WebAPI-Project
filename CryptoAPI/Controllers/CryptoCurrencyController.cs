...
namespace CryptoAPI.Controllers
{
    [Route("api/coins")]
    [ApiController]
    public class CryptoCurrencyController : ControllerBase, IDisposable
    {
        ...
        {
            _context = context;
            var response =  new OnlineMarketCap();
            response.GetOnlineMarketCapAsync().Wait();
        }
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            response.StatusDescription = "Succesfully data retrive.";
        ...
        public async Task<IActionResult> GetAllCategories(int id)
        {
            ...
            var items = await (from category in _context.Currencycategories
                               ...
                               select new
                               {
                                   category.Id,
                                   Category = category.Category,
                                   Coins = _context.Currencysymbols.Where(p=>p.CategoryId == id).Select(p=> new { p.Id,p.Symbol,p.Price}).ToList()
                               }).ToListAsync();
        ...
        public async Task<IActionResult> Add(CurrencysymbolModel currencysymbol)
        {
            ...
            {
                response.StatusCode = 200;
                response.StatusDescription = "Coin Category does not exist exist.";
                return Ok(response);
            }
            ...
            {
                response.StatusCode = 200;
                response.StatusDescription = "Coin already exist.";
                return Ok(response);
            }
            ...
            response.StatusCode = 200;
            response.StatusDescription = "Coin added succesfully retrive.";
            return Ok(response);
        }
